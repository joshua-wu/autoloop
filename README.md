...
autoloop 的效果高度依赖你对任务的描述质量。描述时抓住两个重点：

**1. 任务目标 — 你想要什么结果？**

不要只说"改善代码"或"优化文档"，而是描述最终"做好了"是什么样子。

| 模糊（效果差） | 具体（效果好） |
|---------------|---------------|
| 改善代码质量 | 让 csv_analyzer.py 通过所有 24 个 pytest 测试，修复资源泄漏和排序逻辑 |
| 优化文档 | 补全 README 中缺失的配置项说明，确保与 templates/config.md 一致 |
| 做一份调研 | 产出一份 CTR 模型 scaling 的综述，覆盖工业界最新实践，包含 10 个可落地的 idea |

**2. 输出要求 — 你怎么判断做得好不好？**

告诉 autoloop 你的质量标准和底线，它会据此生成评估器的评判规则。

- **量化指标**：测试通过率、lint 错误数、性能指标
- **质量维度**：准确性、完整性、可读性、创新性（按优先级排列）
- **绝对底线**：不能出现的问题（如：测试不能回归、不能删除已有功能、引用必须可查证）

描述越具体，生成器知道该改什么，评估器知道该查什么，循环效果越好。
