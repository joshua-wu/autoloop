...
Dispatcher 启动时会收到一个**配置文件路径**（如 `config.md` 或 `config-doc-optimize.md`）。

从配置文件名推导任务名和运行目录：
- `config.md` → 任务名为空，运行目录 = `_run/`，分支前缀 = `<branch_prefix>/<tag>`
- `config-<task>.md` → 任务名为 `<task>`，运行目录 = `_run/<task>/`，分支前缀 = `<branch_prefix>/<task>/<tag>`

**在本文档后续所有路径中，`{RUN}` 代表运行目录**（即 `_run/` 或 `_run/<task>/`）。读者看到 `{RUN}/dispatcher/state.json` 时，实际路径根据任务名展开。

## Setup

与用户协作完成以下步骤：

1. **阅读配置**：读取配置文件（由 SKILL.md 传入的路径），理解本次任务的目标、范围和约束。
2. **确认 run tag**：提议一个标签（如 `apr25`），创建分支。
   - 默认任务：`<branch_prefix>/<tag>`
   - 命名任务：`<branch_prefix>/<task>/<tag>`
3. **阅读上下文**：读取配置中 `context` 列出的所有文件。
4. **初始化工作空间**：
   - 创建 `{RUN}` 目录及子目录结构：
     ```
     {RUN}/
...
5. **生成任务 prompt**：根据配置文件的内容，**实际读取 `target_files` 和 `readonly_files`**，结合 `_run/generator.md` 和 `_run/evaluator.md` 的通用指令，为本次任务生成两个专属的任务 prompt 文件。这是将用户的简短配置展开为具体工作指令的关键步骤，必须高质量完成。
   ...
   **5b. 生成 `{RUN}/evaluator/task.md`** — 评估器的专属任务要求。包含：
   ...
   **5c. "待补充"标注** — 生成 task.md 时，如果无法从 config 和实际文件中推断出某项内容，**不得编造**，必须标注 `[待补充：<原因>]`。常见场景：
     - 用户的 `task` 描述过于模糊，无法推导出具体评估维度 → 标注并列出需要用户澄清的问题
     - `readonly_files` 中的文件不存在或为空 → 标注该约束来源缺失
     - `evaluation_methods` 中的命令无法判断正常/异常输出 → 标注需要用户说明预期输出
     - 生成完成后，在 task.md 末尾汇总所有 `[待补充]` 条目为一个清单，输出给用户

   **5d. 一致性检查** — 生成两份 task.md 后，检查以下对齐关系：
     - 两者对"任务目标"的理解是否一致（generator 要改进的方向 = evaluator 会评估的维度）
     - evaluator 的评估维度是否覆盖了 generator 可能改动的所有方面（不存在"改了但没人评"的盲区）
     - 如有不一致，修正后再继续

   **5e. 长度约束** — 每份 task.md 不超过 4000 字节。超出时精简措辞、去掉冗余示例，但不能省略评估维度或关键约束。
   ...
   <workdir>/                              ← Git 根目录 = 工作目录
     ...
     _run/                                 ← [自动生成] 运行时产物（.gitignore）
       ...
       dispatcher/                         ← 默认任务的调度器产物
         state.json
         results.jsonl
       generator/                          ← 默认任务的生成器产物
         task.md
         gen/
           summary.md
           round_001.md
       evaluator/                          ← 默认任务的评估器产物
         task.md
         eval/
           summary.md
           round_001.md
       doc-optimize/                       ← 命名任务 "doc-optimize" 的全部产物
         dispatcher/
           state.json
           results.jsonl
         generator/
...
- 配置文件（`config.md` 或 `config-<task>.md`）— 任务配置
...
- **运行目录 `{RUN}` 的路径**（让生成器知道该把记录写到哪个目录下）
- 当前轮次编号 N
...
4. 如果重试次数 < 配置中 `max_fix_attempts`（默认 3）：
...
```bash
echo '{"round":5,"commit":"a1b2c3d","status":"keep","score":72,"description":"补充 /users 接口的请求参数说明"}' >> {RUN}/dispatcher/results.jsonl
```
...
- `description`：一句话概述本轮做了什么（不得超过配置中 `result_max_bytes` 的设定，默认 200 字节）
...
- 单轮总耗时超过配置中 `timeout_minutes` 的设定，杀掉当前 subagent，算 fail。
...
- **prompt 中必须告知 subagent 运行目录 `{RUN}` 的路径**，让它知道该把产物写到哪里。
...
| `evaluator_running` | 评估器正在运行 | 清理残留 worktree。检查 `{RUN}/evaluator/eval/round_NNN.md` 是否存在——若不存在则本轮算 fail；若存在则视为评估已完成（即使内容可能不完整），进入 deciding 阶段重新决策 |
| `deciding` | 评估完成，待决策 | 读取评估报告，重新执行 Step 4 决策逻辑 |
| `recording` | 已决策，待记录 | 检查 `{RUN}/dispatcher/results.jsonl` 最后一行的 round 字段是否等于当前轮次：若已记录则跳过；若未记录则检查 git log 中是否有 `generator_branch` 的 merge commit 来判断 keep/discard，补写记录 |
...
2. 检查 `{RUN}/dispatcher/state.json` 是否存在
3. 如果不存在 → 正常启动，从 `{RUN}/dispatcher/results.jsonl` 行数推断轮次编号
4. 如果存在 → 读取 phase，按上表执行恢复策略
5. 恢复完成后，更新 `{RUN}/dispatcher/state.json` 为 `complete`，进入下一轮

**连续计数器恢复**：连续 discard/fail/skip 计数不存储在 state.json 中，每次恢复时从 `{RUN}/dispatcher/results.jsonl` 尾部重新计算——从最后一行往前扫描，直到遇到不同 status 为止，即为当前连续计数。
